...
- **デスクトップアプリ**（Windows では推奨）: Tauri 製。Windows / macOS / Linux。BLE・シリアル両対応。
...
ありません。**この手順が煩わしい場合、Windows ではデスクトップ版をご利用ください**（OS のペアリング
済み一覧から直接開けるため、この制約を受けません）。Linux / ChromeOS では制約なく動作します。
