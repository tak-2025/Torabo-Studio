...
2. **接続する。** 画面の **Bluetooth** または **USB** を押し、一覧から自分のキーボードを選びます。
   ...
   - **USB ではキーマップ編集しかできません**（独自設定は BLE 専用のため）。
...
- **デスクトップアプリ**: Tauri 製。Windows / macOS / Linux。BLE・シリアル両対応。
...
独自設定は専用の GATT サービス上にあるため、**USB では届きません**（これはデスクトップ版でも
同じです）。USB 接続時は該当タブを非表示にします。
...
Windows でこの手順を毎回踏みたくない場合は、自分でビルドする価値があります。
