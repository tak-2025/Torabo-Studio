...
export const Key = ({
  ...
  return (
    <button
      ...
      onClick={onClick}
    >
      <div className={`absolute text-keycap ${selected ? "text-primary-content" : "z1text-base-content"} opacity-80 top-1 text-nowrap left-1/2 font-light -translate-x-1/2 text-center`}>{shortenHeader(header)}</div>
      {children}
    </button>
  );
};
